...
while (($# >= 1)); do
	case "$1" in
		...
		-i)
			...
			elif [ "$TERMUX_PREFIX" != "/data/data/dev.mutwakil.androidide/files/usr" ]; then
				termux_error_exit "./build-package.sh: option '-i' is available only when TERMUX_APP_PACKAGE is 'dev.mutwakil.androidide'"
			else
				export TERMUX_INSTALL_DEPS=true
			fi
			;;
		-I)
			if [ "$TERMUX_PREFIX" != "/data/data/dev.mutwakil.androidide/files/usr" ]; then
				termux_error_exit "./build-package.sh: option '-I' is available only when TERMUX_APP_PACKAGE is 'dev.mutwakil.androidide'"
			else
				export TERMUX_INSTALL_DEPS=true
